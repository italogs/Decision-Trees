...
void LocalSearch::run()
{
	...
	while(attempt < MAX_ATTEMPTS)
	{
		...
		target_node = std::rand() % nbInternalNodes;
		target_level = getTreeLevelByNode(target_node);

		descendants.clear();
		getAllDescendants(target_node,target_level);
		move1(0,0);
...
void LocalSearch::perturbation()
{
	...
	int range_start = pow(2, level) - 1;
	int range_end = pow(2, level + 1) - 1; 
	target_node = (std::rand() % range_end) + range_start;
	target_level = getTreeLevelByNode(target_node);
	printf("level: %d target_node %d; ",level, target_node);
	printf("nbMisclassified %d;",solution->getMisclassifiedSamples());
	incumbentSolution = new Solution(params);
	perturbationMove(0,0);
	solution->calculateMisclassifiedSamples();
	printf("dps -> nbMisclassified %d\n",solution->getMisclassifiedSamples());
}
...
void LocalSearch::getAllDescendants(int node, int level)
{
	if(level >= params->maxDepth)
		return;
	descendants.push_back(node);
	getAllDescendants(node*2+1, level+1);
	getAllDescendants(node*2+2, level+1);
}